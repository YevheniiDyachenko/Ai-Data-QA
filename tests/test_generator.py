...
def test_generate_static_tests():
    ...
    assert any(t.id == "users_user_id_not_null" for t in tests)
    # Check for uniqueness test on user_id
    assert any(t.id == "users_user_id_unique" for t in tests)

    # Verify SQL content for not null
    not_null_test = next(t for t in tests if t.id == "users_user_id_not_null")
